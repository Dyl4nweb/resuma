      ...
      <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/70 p-3 sm:p-5 shadow-2xl backdrop-blur-xl ring-1 ring-white/10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 sm:pb-4 border-b border-zinc-800/80">
            ...
            {templates.map((t) => {
              ...
              return (
                ...
                >
                  <span>{t.label}</span>
                  {active && (
                    <span className="text-[9px] bg-white/20 px-1.5 py-0.2 rounded font-mono uppercase">
                      {t.tag}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-3">
            ...
            <div className="inline-flex items-center gap-1.5 rounded-md bg-emerald-500/10 px-2 py-1 text-[11px] font-semibold text-emerald-400 border border-emerald-500/20">
        ...
        <div className="relative mt-4 sm:mt-5 bg-zinc-950/80 rounded-xl p-3 sm:p-8 flex justify-center border border-zinc-800/60 overflow-hidden max-h-[580px] sm:max-h-[660px]">
          {/* Subtle gradient overlay at bottom to encourage scrolling/editing */}
          <div className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-transparent z-10 pointer-events-none flex items-end justify-center pb-5">
            <Link
              href="/register"
              className="pointer-events-auto flex items-center gap-2 rounded-xl bg-red-600 px-5 py-2.5 text-xs font-bold text-white shadow-xl shadow-red-600/30 hover:bg-red-500 transition-transform active:scale-95"
            >
              <span>Customize This Resume in Builder</span>
              <ExternalLink className="h-3.5 w-3.5" />
            </Link>
          </div>

          {/* Scaled Live Resume Document */}
          <div className="w-full flex justify-center transform origin-top scale-[0.65] sm:scale-[0.82] lg:scale-[0.92] transition-transform">
            <TemplateRenderer data={previewData} />
          </div>
        </div>

        {/* Bottom Metrics Strip */}
        <div className="mt-3 px-2 flex flex-wrap items-center justify-between text-xs text-zinc-400 gap-2">
