  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
        ...
        {/* Tagline */}
        <h1 className="text-2xl xs:text-3xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-foreground max-w-3xl mx-auto leading-[1.2] sm:leading-[1.15]">
          <span>Craft Resumes That Actually Land Interviews</span>
          <span className="text-[#dc2626]">.</span>
        </h1>

        {/* Short Description */}
        <p className="mt-4 sm:mt-5 text-sm sm:text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
        ...
        <div className="mt-7 sm:mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 w-full sm:w-auto">
          ...
          {!user && (
            <Link
              href="/login"
              className="w-auto min-w-[140px] max-w-[200px] flex items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5 sm:px-6 sm:py-3.5 text-xs sm:text-base font-semibold text-foreground hover:text-accent-foreground hover:bg-muted transition-colors text-center"
            >
              <span>Sign In</span>
            </Link>
          )}
        ...
        <div
          className="mt-12 inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground cursor-pointer transition-colors"
          onClick={() => {
